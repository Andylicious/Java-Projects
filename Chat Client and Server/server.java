...
exit(1);
   ...
   static class usr_mngr {
      ...
      private static Map <String, PrintWriter> usr_sender;
      usr_mngr () {
         usr_online = Collections.synchronizedMap 
            (new HashMap <String, Boolean> ());
         usr_sender = Collections.synchronizedMap 
      ...
      static void add_cw (String user, PrintWriter pw) {
         usr_sender.put (user, pw);
      }

      static PrintWriter rm_cw (String user) {
         PrintWriter pw;
         pw = usr_sender.remove (user);
         return pw;
      }

      static Collection <PrintWriter> get_vals_cw () {
         Collection <PrintWriter> vals = usr_sender.values ();
         return vals;
      }
   }
   ...
   static class verify_user implements Runnable {
      ...
      public void run () {
         try {
            ...
            Thread cl = new Thread (new receiver (opts, client));
            Thread cw = new Thread (new sender (opts, client));
            cl.start ();
            Thread.currentThread ().sleep (1000);
            cw.start ();

         }catch (IOException exn) {
            System.err.printf ("%s: user exists: %s", exn, opts.username);
            exit(1);
         }catch (InterruptedException exn) {
            System.err.println("IOException error");
            exit(1);
   ...
   // a msg to the usr_table and receiver stops listening.
   //
   static class receiver implements Runnable {
      private String user;
      private Socket client;
      receiver (chatter.options opts, Socket client) {
     ...
     //    out.printf ("client listener starting...%n");
         out.printf("%s has entered the chatroom%n",user);
         try {
            Scanner client_in = new Scanner (client.getInputStream ());
            usr_table.add_usr (user);
           // out.printf("%s",user);
       //    if(client_in.hasNextLine()==true) out.println("True");
         //    else out.println("False");
            // out.println("The line is " + client_in.nextLine()); 
